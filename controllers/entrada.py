...
def entr_1():
    for rows in db(db.livro.data == mes+" de "+ano).select(db.livro.id):
        ...
        x = db.livro[a]
        en = x.entrada1
    formulario = SQLFORM.factory(Field('entrada1', 'float', label = 'Entrada 1,  R$ : '), )
...
def entr_2():
    for rows in db(db.livro.data == mes+" de "+ano).select(db.livro.id):
        ...
        x = db.livro[a]
        en = x.entrada2
    formulario = SQLFORM.factory(Field('entrada2', 'float', label = 'Entrada 2,  R$ : '), )
    if formulario.process().accepted:
...
def entr_3():
    for rows in db(db.livro.data == mes+" de "+ano).select(db.livro.id):
        ...
        x = db.livro[a]
        en = x.entrada3
    formulario = SQLFORM.factory(Field('entrada3', 'float', label = 'Entrada 3,  R$ : '), )
    if formulario.process().accepted:
